...
if __name__ == "__main__":
    ...
    login_instance = WI.wdi_login.WDLogin(user=bot_user, pwd=bot_pwd, mediawiki_api_url="http://localhost:8181/w/api.php")
    ...
    for row in csv_reader:
        ...
        for key in row:
            ...
            if key.find(':') > -1 and key[0] == "P":
                ...
                try:
                    ...
                    if data_type.lower() == 'commonsMedia':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDCommonsMedia(value=row[key], prop_nr=PID)
                            data.append(statement)
                    elif data_type.lower() == 'wikibase-item':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDItemID(value=row[key], prop_nr=PID)
                            data.append(statement)
                    elif data_type.lower() == 'wikibase-property':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDProperty(value=row[key], prop_nr=PID)
                            data.append(statement)
                    elif data_type.lower() == 'string':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDString(value=row[key], prop_nr=PID)
                            data.append(statement)
                    elif data_type.lower() == 'monolingualtext':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDMonolingualText(value=row[key], prop_nr=PID)
                            data.append(statement)       
                    elif data_type.lower() == 'external-id':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDExternalID(value=row[key], prop_nr=PID)
                            data.append(statement)                           
                    elif data_type.lower() == 'quantity':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDQuantity(value=row[key], prop_nr=PID)
                            data.append(statement)
                    elif data_type.lower() == 'time':
                            statement = WI.wdi_core.WDTime(time.strftime(row[key]), prop_nr=PID)
                            data.append(statement)
                    elif data_type.lower() == 'url':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDUrl(value=row[key], prop_nr=PID)
                            data.append(statement)       
                    elif data_type.lower() == 'math':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDMath(value=row[key], prop_nr=PID)
                            data.append(statement)       
                    elif data_type.lower() == 'geo-shape':
                        if row[key] is not None and row[key].strip() != '':
                            statement = WI.wdi_core.WDGeoShape(value=row[key], prop_nr=PID)
                            data.append(statement)  
        ...
        try:
            ...
            print('llega hasta el itemengine')
            ...
            wd_item = WI.wdi_core.WDItemEngine(data=data, mediawiki_api_url="http://localhost:8181/w/api.php", sparql_endpoint_url="http://localhost:8282")
            #print('pasa el itemengine:' + wd_item)
            print(wd_item.get_wd_json_representation())
